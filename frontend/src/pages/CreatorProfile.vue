...
<template>
  ...
  <main v-else class="grid grid-cols-5 auto-rows-[25vh] gap-4">
    ...
    <!-- <div class="row-start-4 row-span-2 col-span-2 w-full h-full">
      <img :src="CREATOR.profile.fav_piece_1.path" alt="" class="w-full object-contain max-h-full ">
    </div>
    <div class="row-start-4 col-start-3 items-end flex">
      <h2 class="text-2xl font-bold">{{ CREATOR.profile.fav_piece_1.title.toUpperCase() }}</h2>
    </div>
    <div class="row-start-5 col-start-3 col-span-3 flex items-end">
      <p class="text-2xl font-bold text-justify">{{ CREATOR.profile.fav_piece_1.story.toUpperCase() }}</p>
    </div>

    <div class="row-start-6 col-start-1 items-end flex">
      <h2 class="text-2xl font-bold">{{ CREATOR.profile.fav_piece_2.title.toUpperCase() }}</h2>
    </div>
    <div class="row-start-7 col-start-1 col-span-3 flex items-end">
      <p class="text-2xl font-bold text-justify">{{ CREATOR.profile.fav_piece_2.story.toUpperCase() }}</p>
    </div>

    <div class="row-start-6 row-span-2 col-span-2 col-start-4 w-full h-full">
      <img :src="CREATOR.profile.fav_piece_2.path" alt="" class="w-full object-contain max-h-full ">
    </div> -->


    <div class="row-start-4 col-start-4 col-span-2">
